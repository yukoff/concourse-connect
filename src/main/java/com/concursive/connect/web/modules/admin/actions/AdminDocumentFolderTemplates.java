...
/**
 * Actions for the administration module for managing document folder templates
 *
 * @author Kailash Bhoopalam
 * @created August 12, 2008
 */
public final class AdminDocumentFolderTemplates extends GenericAction {
  ...
  public String executeCommandList(ActionContext context) {
    ...
    try {
      ...
      context.getRequest().setAttribute("documentFolderTemplateList", templateList);
  ...
  public String executeCommandModify(ActionContext context) {
    ...
    try {
      ...
      DocumentFolderTemplate template = (DocumentFolderTemplate) context.getFormBean();
      db = getConnection(context);
      if (template.getId() == -1 && id != null) {
        template = new DocumentFolderTemplate(db, Integer.parseInt(id));
      }
      if (template.getId() > -1 && template.getEntered() != null) {
        template = new DocumentFolderTemplate(db, template.getId());
      }
      context.getRequest().setAttribute("documentFolderTemplate", template);
      // Load project category drop-down list
      ProjectCategoryList projectCategoryList = new ProjectCategoryList();
      projectCategoryList.setTopLevelOnly(true);
      projectCategoryList.buildList(db);
  ...
  public String executeCommandDelete(ActionContext context) {
    ...
    try {
      ...
      DocumentFolderTemplate template = new DocumentFolderTemplate(db, id);
      template.delete(db);
    } catch (Exception e) {
      e.printStackTrace();
      context.getRequest().setAttribute("Error", e);
