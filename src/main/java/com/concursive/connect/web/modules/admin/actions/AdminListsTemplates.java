...
/**
 * Actions for the administration module for managing list templates
 *
 * @author Kailash Bhoopalam
 * @created August 13, 2008
 */
public final class AdminListsTemplates extends GenericAction {
  ...
  public String executeCommandList(ActionContext context) {
    ...
    try {
      ...
      context.getRequest().setAttribute("listsTemplateList", templateList);
  ...
  public String executeCommandModify(ActionContext context) {
    ...
    try {
      ...
      ListsTemplate template = (ListsTemplate) context.getFormBean();
      db = getConnection(context);
      if (template.getId() == -1 && id != null) {
        template = new ListsTemplate(db, Integer.parseInt(id));
      }
      if (template.getId() > -1 && template.getEntered() != null) {
        template = new ListsTemplate(db, template.getId());
      }
      context.getRequest().setAttribute("listsTemplate", template);
      // Load project category drop-down list
      ProjectCategoryList projectCategoryList = new ProjectCategoryList();
      projectCategoryList.setTopLevelOnly(true);
      projectCategoryList.buildList(db);
  ...
  public String executeCommandDelete(ActionContext context) {
    ...
    try {
      ...
      ListsTemplate template = new ListsTemplate(db, id);
      template.delete(db);
    } catch (Exception e) {
      e.printStackTrace();
      context.getRequest().setAttribute("Error", e);
