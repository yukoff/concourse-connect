...
import com.concursive.connect.web.modules.communications.dao.MessageTemplateList;
...
public final class AdminProjectMessageTemplates extends GenericAction {
  ...
  public String executeCommandList(ActionContext context) {
    ...
    try {
      ...
      // Get templates
      MessageTemplateList templateList = new MessageTemplateList();
      templateList.buildList(db);
      context.getRequest().setAttribute("messageTemplateList", templateList);
  ...
  public String executeCommandDetails(ActionContext context) {
    ...
    Connection db = null;
    try {
      int templateId = Integer.parseInt(context.getRequest().getParameter("id"));
      db = getConnection(context);
      // Load the template
      MessageTemplate template = new MessageTemplate(db, templateId);
      context.getRequest().setAttribute("messageTemplate", template);
      // Load project category drop-down list
      ProjectCategoryList projectCategoryList = new ProjectCategoryList();
      projectCategoryList.setTopLevelOnly(true);
      projectCategoryList.buildList(db);
  ...
  public String executeCommandModify(ActionContext context) {
    ...
    try {
      ...
      MessageTemplate template = (MessageTemplate) context.getFormBean();
      db = getConnection(context);
      if (template.getId() == -1 && id != null) {
        template = new MessageTemplate(db, Integer.parseInt(id));
      }
      if (template.getId() > -1 && template.getEntered() != null) {
        template = new MessageTemplate(db, template.getId());
      }
      context.getRequest().setAttribute("messageTemplate", template);
      // Load project category drop-down list
      ProjectCategoryList projectCategoryList = new ProjectCategoryList();
  ...
  public String executeCommandDelete(ActionContext context) {
    ...
    try {
      ...
      MessageTemplate template = new MessageTemplate(db, id);
      template.delete(db);
    } catch (Exception e) {
      e.printStackTrace();
      context.getRequest().setAttribute("Error", e);
