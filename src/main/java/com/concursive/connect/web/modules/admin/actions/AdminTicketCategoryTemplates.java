...
/**
 * Actions for the administration module to manage ticket category templates
 *
 * @author Kailash Bhoopalam
 * @created February 20, 2008
 */
public final class AdminTicketCategoryTemplates extends GenericAction {
  ...
  public String executeCommandList(ActionContext context) {
    ...
    try {
      ...
      context.getRequest().setAttribute("ticketCategoryTemplateList", templateList);
  ...
  public String executeCommandModify(ActionContext context) {
    ...
    try {
      ...
      TicketCategoryTemplate template = (TicketCategoryTemplate) context.getFormBean();
      db = getConnection(context);
      if (template.getId() == -1 && id != null) {
        template = new TicketCategoryTemplate(db, Integer.parseInt(id));
      }
      if (template.getId() > -1 && template.getEntered() != null) {
        template = new TicketCategoryTemplate(db, template.getId());
      }
      context.getRequest().setAttribute("ticketCategoryTemplate", template);
      // Load project category drop-down list
      ProjectCategoryList projectCategoryList = new ProjectCategoryList();
      projectCategoryList.setTopLevelOnly(true);
      projectCategoryList.buildList(db);
  ...
  public String executeCommandDelete(ActionContext context) {
    ...
    try {
      ...
      TicketCategoryTemplate template = new TicketCategoryTemplate(db, id);
      template.delete(db);
    } catch (Exception e) {
      e.printStackTrace();
      context.getRequest().setAttribute("Error", e);
