 ...
 * @created July 9, 2008
 */
public final class AdminWikiTemplates extends GenericAction {
  ...
  public String executeCommandList(ActionContext context) {
    ...
    try {
      ...
      context.getRequest().setAttribute("wikiTemplateList", templateList);
  ...
  public String executeCommandDetails(ActionContext context) {
    ...
    try {
      ...
      db = getConnection(context);
      // Load the template
      WikiTemplate template = new WikiTemplate(db, templateId);
      context.getRequest().setAttribute("wikiTemplate", template);
      // Load project category drop-down list
      ProjectCategoryList projectCategoryList = new ProjectCategoryList();
      projectCategoryList.setTopLevelOnly(true);
      projectCategoryList.buildList(db);
  ...
  public String executeCommandModify(ActionContext context) {
    ...
    try {
      ...
      WikiTemplate template = (WikiTemplate) context.getFormBean();
      db = getConnection(context);
      if (template.getId() == -1 && id != null) {
        template = new WikiTemplate(db, Integer.parseInt(id));
      }
      if (template.getId() > -1 && template.getEntered() != null) {
        template = new WikiTemplate(db, template.getId());
      }
      context.getRequest().setAttribute("wikiTemplate", template);
      // Load project category drop-down list
      ProjectCategoryList projectCategoryList = new ProjectCategoryList();
  ...
  public String executeCommandDelete(ActionContext context) {
    ...
    try {
      ...
      WikiTemplate template = new WikiTemplate(db, id);
      template.delete(db);
    } catch (Exception e) {
      e.printStackTrace();
      context.getRequest().setAttribute("Error", e);
