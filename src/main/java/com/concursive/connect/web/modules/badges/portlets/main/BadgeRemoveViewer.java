...
/**
 * Project remove badge ajax call
 *
 * @author matt rajkowski
 * @created December 30, 2008
 */
public class BadgeRemoveViewer implements IPortletViewer {
  ...
  public String doView(RenderRequest request, RenderResponse response) throws Exception {
    ...
    if (projectBadge.getProjectId() != project.getId()) {
      throw new PortletException("Project id mismatch");
    }

    // Load the badge for display
    Badge badge = BadgeUtils.loadBadge(projectBadge.getBadgeId());
    request.setAttribute(BADGE, badge);

    // @todo move this logic to BadgeUtils
    // Delete the badge mapping
    if (user.getAccessAdmin() || !badge.getSystemAssigned()) {
    ...
    request.setAttribute(BADGE_MEMBER_COUNT, memberCountValue);
