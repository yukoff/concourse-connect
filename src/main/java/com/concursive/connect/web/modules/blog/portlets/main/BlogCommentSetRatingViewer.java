...
/**
 * Project blog comment rating
 *
 * @author Kailash Bhoopalam
 * @created February 25, 2008
 */
public class BlogCommentSetRatingViewer implements IPortletViewer {
  ...
  public String doView(RenderRequest request, RenderResponse response)
    ...
    BlogPostComment thisBlogPostComment = new BlogPostComment(db, recordId, -1);
    BlogPost blogPost = new BlogPost(db, thisBlogPostComment.getNewsId());
    if (blogPost.getProjectId() != project.getId()) {
      throw new PortletException("PermissionError");
    }
    // The author of a wiki comment should not be able to rate it
    if (thisBlogPostComment.getEnteredBy() == user.getId()) {
      throw new PortletException("PermissionError");
    }
    ...
    RatingBean thisRating =
        Rating.save(db, user.getId(), project.getId(), thisBlogPostComment.getId(), vote, BlogPostComment.TABLE, Comment.PRIMARY_KEY, Constants.UNDEFINED);
