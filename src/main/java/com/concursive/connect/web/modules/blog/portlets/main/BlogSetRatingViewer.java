...
/**
 * Blog rating ajax command
 *
 * @author matt rajkowski
 * @created October 31, 2008
 */
public class BlogSetRatingViewer implements IPortletViewer {
  ...
  public String doView(RenderRequest request, RenderResponse response) throws Exception {
    ...
    // The author of a blog should not be able to rate it
    if (thisArticle.getEnteredBy() == user.getId()) {
      throw new PortletException("PermissionError");
    }
    ...
    RatingBean thisRating =
        Rating.save(db, user.getId(), project.getId(), thisArticle.getId(), vote, BlogPost.TABLE, BlogPost.PRIMARY_KEY, Constants.UNDEFINED);
