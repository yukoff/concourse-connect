...
/**
 * Calendar Events rating ajax command
 *
 * @author Kailash Bhoopalam
 * @created February 26, 2009
 */
public class EventSetInappropriateViewer implements IPortletViewer {
  ...
  public String doView(RenderRequest request, RenderResponse response)
    ...
    Meeting thisMeeting = new Meeting(db, recordId);
    // The author of a topic should not be able to rate it
    if (thisMeeting.getEnteredBy() == user.getId()) {
    ...
    RatingBean thisRating =
        Rating.save(db, user.getId(), project.getId(), thisMeeting.getId(), vote, Meeting.TABLE, Meeting.PRIMARY_KEY, Constants.UNDEFINED);
