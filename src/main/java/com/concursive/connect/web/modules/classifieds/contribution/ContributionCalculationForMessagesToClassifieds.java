...
package com.concursive.connect.web.modules.classifieds.contribution;
...
/**
 * Class for calculating user contributions based on the number classifieds that have received a message
 *
...
public class ContributionCalculationForMessagesToClassifieds extends ContributionCalculation {
  ...
  public void process(Connection db, LookupContribution lookupContribution) throws SQLException {
    ...
    privateMessageList.setLinkModuleId(Constants.PROJECT_CLASSIFIEDS_FILES);
    ...
    ArrayList<Integer> classifiedList = new ArrayList<Integer>();

    while (privateMessageList.size() > 0) {
      for (PrivateMessage privateMessage : privateMessageList) {
        int classifiedId = privateMessage.getLinkItemId();
        //Insure that an classified is counted only once
        if (!classifiedList.contains(classifiedId)) {
          classifiedList.add(classifiedId);
          Classified classified = new Classified(db, classifiedId);
          int userId = classified.getEnteredBy();
