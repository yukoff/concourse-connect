...
/**
 * Represents a Classified Category in iTeam
 *
 * @author Kailash Bhoopalam
 * @version $Id$
 * @created May 21, 2008
 */
public class ClassifiedCategory extends GenericBean {

  public static final String TABLE = "classified_category";
  ...
  /**
   * Constructor for the ClassifiedCategory object
   */
   ...
   */
  public ClassifiedCategory(ResultSet rs) throws SQLException {
    buildRecord(rs);
  }
   ...
   */
  public ClassifiedCategory(Connection db, int thisId) throws SQLException {
  ...
  public boolean insert(Connection db) throws SQLException {
    ...
    try {
      ...
      pst.execute();
      pst.close();
      id = DatabaseUtils.getCurrVal(db, "classified_category_code_seq", id);
  ...
  public boolean delete(Connection db, String basePath) throws SQLException {
    ...
    try {
      ...
      PreparedStatement pst = db.prepareStatement(
          "DELETE FROM classified_category " +
              "WHERE code = ? ");
      pst.setInt(1, id);
  ...
  /**
   * Gets the valid attribute of the ClassifiedCategory object
   *
  ...
  public void saveAttachments(Connection db, int userId, String basePath) throws SQLException {
    if (StringUtils.hasText(this.getAttachmentList())) {
      if (this.getLogoId() != -1) {
        int classifiedCategoryLogoId = this.getLogoId();
        this.setLogoId(-1);
        this.update(db);

        FileItem fileItem = new FileItem(db, classifiedCategoryLogoId);
        fileItem.delete(db, basePath);
      }
      FileItemList.convertTempFiles(db, Constants.CLASSIFIED_CATEGORY_FILES, userId, this.getId(), attachmentList);
