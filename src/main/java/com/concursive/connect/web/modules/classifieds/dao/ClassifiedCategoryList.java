 ...
 * @created May 13, 2008
 */
public class ClassifiedCategoryList extends ArrayList<ClassifiedCategory> {
  // main classified category filters
  ...
  /**
   * Constructor for the ClassifiedCategoryList object
   */
  public ClassifiedCategoryList() {
  }
  ...
  /**
   * Sets the pagedListInfo attribute of the ClassifiedCategoryList object
   *
  ...
  /**
   * @param code the classifiedId to set
   */
  ...
  public void buildList(Connection db) throws SQLException {
    ...
    sqlCount.append(
        "SELECT COUNT(*) AS recordcount " +
            "FROM classified_category cc " +
            "LEFT JOIN lookup_project_category lpc ON (lpc.code = cc.project_category_id) " +
            "WHERE cc.code > -1 ");
    ...
    while (rs.next()) {
      ...
      ++count;
      ClassifiedCategory thisClassifiedCategory = new ClassifiedCategory(rs);
      this.add(thisClassifiedCategory);
    }
    rs.close();
    pst.close();
  ...
  private void createFilter(Connection db, StringBuffer sqlFilter) {
    ...
    if (code > -1) {
      sqlFilter.append("AND (cc.code = ?) ");
    }
    if (projectCategoryId > -1) {
      sqlFilter.append("AND cc.project_category_id = ? ");
    }
    if (enabled != Constants.UNDEFINED) {
      sqlFilter.append("AND (cc.enabled = ?) ");
    }
    if (onlyWithoutProjectCategory) {
      sqlFilter.append("AND cc.project_category_id IS NOT NULL ");
    }
    if (categoryLowercaseName != null) {
      sqlFilter.append("AND " + DatabaseUtils.toLowerCase(db, "cc.item_name") + " = ?  ");
    }
  ...
  /**
   * Gets the htmlSelect attribute of the ClassifiedCategoryList object
   *
  ...
  public void delete(Connection db, String basePath) throws SQLException {
    Iterator<ClassifiedCategory> classifiedCategoryItr = this.iterator();
    while (classifiedCategoryItr.hasNext()) {
      ClassifiedCategory classifiedCategory = classifiedCategoryItr.next();
      classifiedCategory.delete(db, basePath);
    }
  }
  ...
  private void buildLogos(Connection db) throws SQLException {
    ...
    while (classifiedCategoryItr.hasNext()) {
      ClassifiedCategory classifiedCategory = classifiedCategoryItr.next();
      classifiedCategory.buildLogo(db);
    }
  }
