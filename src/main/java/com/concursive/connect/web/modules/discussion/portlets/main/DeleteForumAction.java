...
/**
 * Action for deleting a forum
 *
 * @author matt rajkowski
 * @created Nov 12, 2008 9:20:28 AM
 */
public class DeleteForumAction implements IPortletAction {

  // Logger
  private static Log LOG = LogFactory.getLog(DeleteForumAction.class);
  ...
  public GenericBean processAction(ActionRequest request, ActionResponse response) throws Exception {
    ...
    if (!ProjectUtils.hasAccess(project.getId(), user, "project-discussion-forums-delete")) {
    ...
    int id = -1;
    String idValue = request.getParameter("forum");
    if (idValue != null) {
    ...
    forum.delete(db, getFileLibraryPath(request, "projects"));

    // Remove from index
    indexDeleteItem(request, forum);

    // Trigger any workflow
    PortalUtils.processDeleteHook(request, forum);
