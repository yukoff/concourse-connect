...
/**
 * Action for deleting a forum topic reply
 *
 * @author matt rajkowski
 * @created Nov 14, 2008 9:20:28 AM
 */
public class DeleteReplyAction implements IPortletAction {
  ...
  public GenericBean processAction(ActionRequest request, ActionResponse response) throws Exception {
    ...
    int id = -1;
    String idValue = request.getParameter("reply");
    if (idValue != null) {
    ...
    reply.delete(db, getFileLibraryPath(request, "projects"));

    // Remove from index
    indexDeleteItem(request, reply);

    // Trigger any workflow
    PortalUtils.processDeleteHook(request, reply);

    // This call will close panels and perform redirects
    return (PortalUtils.performRefresh(request, response, "/show/topic/" + reply.getIssueId()));
  }
}
