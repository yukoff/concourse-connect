...
/**
 * Action for saving a forum topic
 *
 * @author matt rajkowski
 * @created Nov 12, 2008 9:20:28 AM
 */
public class SaveTopicAction implements IPortletAction {
  ...
  public GenericBean processAction(ActionRequest request, ActionResponse response) throws Exception {
    ...
    Topic topic = (Topic) getFormBean(request, Topic.class);

    // Set default values when saving records
    topic.setProjectId(project.getId());
    topic.setModifiedBy(user.getId());

    // Save the record
    if (topic.getId() == -1) {
      // This is a new record
      if (!ProjectUtils.hasAccess(project.getId(), user, "project-discussion-topics-add")) {
        throw new PortletException("Unauthorized to add in this project");
      }
      topic.setEnteredBy(user.getId());
      recordInserted = topic.insert(db);
    } else {
      // This is an existing record
      if (!ProjectUtils.hasAccess(project.getId(), user, "project-discussion-topics-edit")) {
        throw new PortletException("Unauthorized to edit in this project");
      }
      resultCount = topic.update(db);
    }
    ...
    // Index the record
    PortalUtils.indexAddItem(request, topic);

    // Trigger any workflow
    PortalUtils.processInsertHook(request, topic);
