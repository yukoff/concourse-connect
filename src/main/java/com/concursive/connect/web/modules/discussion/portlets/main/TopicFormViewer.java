...
/**
 * Generates a forum topic form
 *
 * @author matt rajkowski
 * @created November 12, 2008
 */
public class TopicFormViewer implements IPortletViewer {

  // Pages
  private static final String VIEW_PAGE = "/projects_center_issues_add.jsp";
  ...
  public String doView(RenderRequest request, RenderResponse response) throws Exception {
    ...
    if (recordId == -1 && !ProjectUtils.hasAccess(project.getId(), user, "project-discussion-topics-add")) {
      throw new PortletException("Unauthorized to add this record");
    } else if (recordId > -1 && !ProjectUtils.hasAccess(project.getId(), user, "project-discussion-topics-edit")) {
    ...
    // Load the record if modifying
    if (recordId > -1) {
      thisTopic = new Topic(db, recordId, project.getId());
      request.setAttribute(TOPIC, thisTopic);
    } else {
      // Determine the forum when adding
      String forumValue = request.getParameter("forum");
      if (forumValue != null) {
        int forumId = Integer.parseInt(forumValue);
        thisTopic.setCategoryId(forumId);
      }
    }
