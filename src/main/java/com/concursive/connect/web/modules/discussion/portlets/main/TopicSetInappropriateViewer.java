...
/**
 * Topic rating ajax command
 *
 * @author Kailash Bhoopalam
 * @created February 24, 2009
 */
public class TopicSetInappropriateViewer implements IPortletViewer {
  ...
  public String doView(RenderRequest request, RenderResponse response) throws Exception {
    ...
    Topic thisTopic = new Topic(db, recordId, project.getId());
    if (thisTopic.getProjectId() != project.getId()) {
      throw new PortletException("PermissionError");
    }
    // The author of a topic should not be able to rate it
    if (thisTopic.getEnteredBy() == user.getId()) {
      throw new PortletException("PermissionError");
    }
    ...
    RatingBean thisRating =
        Rating.save(db, user.getId(), project.getId(), thisTopic.getId(), vote, Topic.TABLE, Topic.PRIMARY_KEY, Constants.UNDEFINED);
