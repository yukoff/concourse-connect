 ...
 */
public class DeleteFolderAction implements IPortletAction {
  ...
  public GenericBean processAction(ActionRequest request, ActionResponse response) throws Exception {
    ...
    if (!ProjectUtils.hasAccess(project.getId(), user, "project-documents-folders-delete")) {
    ...
    // Load the record and delete the folder and all contents
    FileFolder thisFolder = new FileFolder(db, id, project.getId(), Constants.PROJECTS_FILES);
    thisFolder.delete(db, filePath);

    // Remove from index
    indexDeleteItem(request, thisFolder);

    // Trigger the workflow
    processDeleteHook(request, thisFolder);
    ...
    if (thisFolder.getParentId() == -1) {
      ...
      return (PortalUtils.performRefresh(request, response, "/show/folder/" + thisFolder.getParentId()));
    }
  }
