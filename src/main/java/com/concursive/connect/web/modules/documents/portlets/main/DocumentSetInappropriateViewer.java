...
/**
 * Documents rating ajax command
 *
 * @author Kailash Bhoopalam
 * @created February 26, 2009
 */
public class DocumentSetInappropriateViewer implements IPortletViewer {
  ...
  public String doView(RenderRequest request, RenderResponse response)
    ...
    if (!ProjectUtils.hasAccess(project.getId(), user, "project-documents-files-download")) {
    ...
    FileItem thisFileItem = new FileItem(db, recordId, project.getId(), Constants.PROJECTS_FILES);
    // The author of a topic should not be able to rate it
    if (thisFileItem.getEnteredBy() == user.getId()) {
    ...
    RatingBean thisRating =
        Rating.save(db, user.getId(), project.getId(), thisFileItem.getId(), vote, FileItem.TABLE, FileItem.PRIMARY_KEY, Constants.UNDEFINED);
