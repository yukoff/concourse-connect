 ...
 */
public class MoveFolderAction implements IPortletAction {
  ...
  public GenericBean processAction(ActionRequest request, ActionResponse response) throws Exception {
    ...
    if (newParentId != -1) {
      new FileFolder(db, newParentId, project.getId(), Constants.PROJECTS_FILES);
    }

    // Update the folder
    thisFolder.updateParentId(db, newParentId);

    // @todo reindex the contained files

    // This call will close panels and perform redirects
    return (PortalUtils.performRefresh(request, response, "/show/folder/" + thisFolder.getId()));
  }
