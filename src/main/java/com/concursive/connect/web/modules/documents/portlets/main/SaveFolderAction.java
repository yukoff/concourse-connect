 ...
 */
public class SaveFolderAction implements IPortletAction {
  ...
  public GenericBean processAction(ActionRequest request, ActionResponse response) throws Exception {
    ...
    FileFolder thisRecord = (FileFolder) getFormBean(request, FileFolder.class);
    ...
    if (thisRecord.getId() == -1) {
      ...
      if (!ProjectUtils.hasAccess(project.getId(), user, "project-documents-folders-add")) {
        throw new PortletException("Unauthorized to add in this project");
      }
      // Prevent hierarchical folder structures to avoid user confusion
      thisRecord.setParentId(-1);

      thisRecord.setEnteredBy(user.getId());
    ...
    if (recordInserted) {
      ...
      PortalUtils.processInsertHook(request, thisRecord);
    }
    ...
    return (PortalUtils.performRefresh(request, response, "/show/folder/" + thisRecord.getId()));
  }
}