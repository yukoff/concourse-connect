...
/**
 * Contains a collection of list templates
 *
 * @author Kailash Bhoopalam
 * @created August 13, 2008
 */
public class ListsTemplateList extends ArrayList<ListsTemplate> {

  private int listTemplateId = -1;
  ...
  public void setListTemplateId(int listTemplateId) {
    this.listTemplateId = listTemplateId;
  }

  public void setListTemplateId(String tmp) {
    this.listTemplateId = Integer.parseInt(tmp);
  }
  ...
  public void buildList(Connection db) throws SQLException {
    ...
    pagedListInfo.setDefaultSort("lt.entered", null);
    ...
    while (rs.next()) {
      ...
      ++count;
      ListsTemplate thisRecord = new ListsTemplate(rs);
      this.add(thisRecord);
    }
    rs.close();
    pst.close();
  ...
  protected void createFilter(StringBuffer sqlFilter) {
    if (listTemplateId > -1) {
      sqlFilter.append("AND lt.template_id = ? ");
    }
    if (projectCategoryId > -1) {
      sqlFilter.append("AND lt.project_category_id = ? ");
    }
    if (enabled != Constants.UNDEFINED) {
      sqlFilter.append("AND lt.enabled = ? ");
    }
  }
