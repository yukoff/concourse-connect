...
public class RequirementIndexer implements Indexer {
  ...
  public void add(IIndexerService writer, Connection db, IndexerContext context) throws SQLException, IOException {
    ...
    while (rs.next() && context.getEnabled()) {
      ...
      // add the document
      writer.indexAddItem(requirement, false);
    }
    rs.close();
    pst.close();
  ...
  public void add(IndexWriter writer, Object item, boolean modified) throws IOException {
    ...
    document.add(new Field("contents",
        requirement.getShortDescription() + " " +
            ...
            requirement.getDepartmentBy(), Field.Store.YES, Field.Index.TOKENIZED));
    if (modified) {
  ...
  public Term getSearchTerm(Object item) {
    Requirement requirement = (Requirement) item;
    return new Term("requirementKeyId", String.valueOf(requirement.getId()));
  }
  ...
  public Term getDeleteTerm(Object item) {
    Requirement requirement = (Requirement) item;
    return new Term("requirementId", String.valueOf(requirement.getId()));
  }
}
