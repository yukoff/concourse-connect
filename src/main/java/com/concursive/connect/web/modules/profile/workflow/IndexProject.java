 ...
 * @created May 19, 2010
 */
public class IndexProject extends ObjectHookComponent implements ComponentInterface {

  private static Log LOG = LogFactory.getLog(SetProjectPermission.class);

  public String getDescription() {
    return "Indexes the specified project";
  }

  public boolean execute(ComponentContext context) {
    // Access the scheduler
    Scheduler scheduler = (Scheduler) context.getAttribute(ComponentContext.SCHEDULER);
    IndexEvent indexEvent = null;

    // Determine the index event type
    if (context.isInsert() || context.isUpdate()) {
      // Update the index
      Project project = (Project) context.getThisObject();
      indexEvent = new IndexEvent(project, IndexEvent.ADD);
    } else if (context.isDelete()) {
      // Return from the index
      Project project = (Project) context.getPreviousObject();
      indexEvent = new IndexEvent(project, IndexEvent.DELETE);
    }
    ...
    try {
      ...
      LOG.error("Could not trigger indexer");
      return false;
    }
    return true;
  }
