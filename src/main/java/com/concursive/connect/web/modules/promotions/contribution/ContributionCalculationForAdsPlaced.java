...
/**
 * Class for calculating user contributions based on the number of ads placed
 *
...
public class ContributionCalculationForAdsPlaced extends ContributionCalculation {
  ...
  public void process(Connection db, LookupContribution lookupContribution) throws SQLException {
    ...
    adList.setPublishedRangeStart(lastRun);
    adList.setPublishedRangeEnd(currentRun);
    adList.buildList(db);

    while (adList.size() > 0) {
      for (Ad projectRating : adList) {
