...
package com.concursive.connect.web.modules.promotions.utils;
...
/**
 * Counts ads by category, dates, author, etc.
 *
...
public class AdCounter {
  ...
  public void buildCounts(Connection db) throws SQLException {
    ...
    for (Ad thisAd : adList) {
      if (thisAd.getPublishDate() != null) {
        dates.add(thisAd.getPublishDate());
      } else {
        dates.addNullCountsForKey("Unpublished");
      }
      authors.add(String.valueOf(thisAd.getEnteredBy()));
      categories.add(String.valueOf(thisAd.getCategoryId()));
      ++total;
    }
  }
