 ...
 * @created July 2, 2008
 */
public class ProjectRatingIndexer implements Indexer {
  ...
  public void add(IIndexerService writer, Connection db, IndexerContext context) throws SQLException, IOException {
    ...
    PreparedStatement pst = db.prepareStatement(
        "SELECT rating_id, project_id, title, comment, modified " +
    ...
    while (rs.next() && context.getEnabled()) {
      ...
      projectRating.setId(rs.getInt("rating_id"));
      projectRating.setProjectId(rs.getInt("project_id"));
      projectRating.setTitle(rs.getString("title"));
      projectRating.setComment(rs.getString("comment"));
      projectRating.setModified(rs.getTimestamp("modified"));
      // add to index
      writer.indexAddItem(projectRating, false);
    }
    rs.close();
    pst.close();
    if (System.getProperty("DEBUG") != null) {
      System.out.println("ProjectRatingIndexer-> Finished: " + count);
    }
  }
  ...
  public void add(IndexWriter writer, Object item, boolean modified) throws IOException {
    ...
    document.add(new Field("ratingId", String.valueOf(projectRating.getId()), Field.Store.YES, Field.Index.UN_TOKENIZED));
    document.add(new Field("projectId", String.valueOf(projectRating.getProjectId()), Field.Store.YES, Field.Index.UN_TOKENIZED));
    if (projectRating.getProjectId() > -1) {
      ...
      if (membership == 1 && ProjectUtils.hasAccess(project.getId(), UserUtils.createGuestUser(), "project-reviews-view")) {
        membership = 0;
      }
    ...
    if (projectRating.getModified() != null) {
      ...
      document.add(new Field("lastModified", String.valueOf(formatter.format(projectRating.getModified())), Field.Store.YES, Field.Index.UN_TOKENIZED));
    }
  ...
  /**
   * Gets the unique searchTerm attribute of the ReviewsIndexer class
   *
   * @param item Description of the Parameter
  ...
  public Term getSearchTerm(Object item) {
    ProjectRating projectRating = (ProjectRating) item;
    return new Term("ratingId", String.valueOf(projectRating.getId()));
  }


  /**
   * Gets the deleteTerm attribute of the ReviewsIndexer class
   *
   * @param item Description of the Parameter
