 ...
 */
public class ReviewsSetRatingViewer implements IPortletViewer {

  // Pages
  private static final String VIEW_PAGE = "/message_true.jsp";

  private static Log LOG = LogFactory.getLog(ReviewsSetRatingViewer.class);
  ...
  public String doView(RenderRequest request, RenderResponse response) throws Exception {
    ...
    // Make sure it matches the project in which permissions were already checked
    if (thisProjectRating.getProjectId() != project.getId()) {
      throw new PortletException("PermissionError");
    }

    // The author of a review can edit a rating only if he has admin privileges
    if (thisProjectRating.getEnteredBy() == user.getId() && !ProjectUtils.hasAccess(project.getId(), user, "project-reviews-admin")) {
      throw new PortletException("PermissionError");
    }

    // Save the rating for this user, or update their rating
    Rating.save(db, user.getId(), project.getId(), thisProjectRating.getId(), vote, ProjectRating.TABLE, ProjectRating.PRIMARY_KEY, Constants.FALSE);
    return defaultView;
  }
}