...
public class Diff {
  ...
  public static void main(String argstrings[]) {
    ...
    File oldFile = new File(argstrings[0]);
    File newFile = new File(argstrings[1]);
    Diff d = new Diff();
    try {
      System.out.println(d.doDiff(oldFile, newFile));
    } catch (IOException e) {
      System.err.println("Diff-> main: " + e.getMessage());
    }
    return;
  }
   ...
   */
  public String doDiff(File oldFile, File newFile) throws IOException {
    println(
        ">>>> Difference of file \"" + oldFile +
            "\" and file \"" + newFile + "\".\n");
  ...
  public String doDiff(String oldContent, String newContent) {
    oldinfo = new fileInfo(oldContent);
    newinfo = new fileInfo(newContent);
    try {
      inputscan(oldinfo);
      inputscan(newinfo);
    } catch (IOException e) {
   ...
   */
  void storeline(String linebuffer, fileInfo pinfo) throws IOException {
    ...
    if (linenum > fileInfo.MAXLINECOUNT) {
      ...
      throw new IOException("MAXLINECOUNT exceeded, must stop.");
    }
  ...
  void showdelete() {
    ...
    printstatus = delete;
    println(oldinfo.symbol[printoldline].showSymbol());
    anyprinted = true;
    printoldline++;
  }
  ...
  void showinsert() {
    ...
    printstatus = insert;
    println(newinfo.symbol[printnewline].showSymbol());
    anyprinted = true;
    printnewline++;
  }
...
class node {                       /* the tree is made up of these nodes */
   ...
   *
   * @param pline A line of the text file
   */
  node(String pline) {
    pleft = pright = null;
    linestate = freshnode;
