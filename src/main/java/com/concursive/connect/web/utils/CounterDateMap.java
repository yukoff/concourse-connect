...
/**
 * Stores dates (year-month) and a related occurrence count
 *
 * @author matt rajkowski
 * @created Apr 23, 2008
 */
public class CounterDateMap extends HashMap<String, Integer> {
  ...
  public void add(Date date) {
    ...
    String monthString = month < 10 && Integer.toString(month).length() == 1 ? "0" + month : Integer.toString(month);
    String key = year + "-" + monthString;
  ...
  public void addNullCountsForKey(String key) {
    if (this.containsKey(key)) {
      this.put(key, this.get(key) + 1);
    } else {
      this.put(key, 1);
