    ...
    <table width="648" border="0" cellpadding="0" cellspacing="0" style="margin:25px auto; " align="center">
      ...
      <tr>
        <td style="padding:10px 20px; background:#fff; border-right:1px solid #e2eaf8; border-left:1px solid #e2eaf8">
          <p>${host.nameFirstLast?html} has <strong>${status?html}</strong> your request to join <#if meeting.isDimdim>web meeting </#if>event, ${meeting.title?html}, of ${startDate?html}.</p>
          <p>You can view more information about the event here:<br />
