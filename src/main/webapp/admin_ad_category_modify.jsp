...
<jsp:useBean id="adCategory" class="com.concursive.connect.web.modules.promotions.dao.AdCategory" scope="request" />
...
<a href="<%= ctx %>/AdminAdCategories.do?command=List">Ad Categories</a> >
<% if (adCategory.getId() != -1) { %>
Modify Ad Category
<% } else { %>
Add Ad Category
<% } %>
...
<script type="text/javascript" language="JavaScript">
  ...
  function checkForm(form) {
    ...
    if (formTest == false) {
      messageText = "The ad category form could not be submitted.          \r\nPlease verify the following items:\r\n\r\n" + messageText;
...
<form name="modifyAdCategory" method="post" action="<%= ctx %>/AdminAdCategories.do?command=Save&auto-populate=true" onSubmit="return checkForm(this);">
  <table class="pagedList">
    ...
    <tbody>
      ...
      <td>
        <%= projectCategoryList.getHtmlSelect("projectCategoryId", adCategory.getProjectCategoryId()) %>
      </td>
      <tr class="containerBody">
        <td nowrap class="formLabel"><ccp:label name="">Name</ccp:label></td>
        <td>
          <input type="text" name="itemName" value="<%=toHtmlValue(adCategory.getItemName()) %>"/><span class="required">*</span>
  ...
  <input type="hidden" name="id" value="<%= adCategory.getId() %>">
  <input type="hidden" name="dosubmit" value="true">
  <input type="submit" value="<ccp:label name="button.save">Save</ccp:label>">
  <input type="button" value="<ccp:label name="button.cancel">Cancel</ccp:label>" onClick="this.form.dosubmit.value='false';window.location.href='<%= ctx %>/AdminAdCategories.do?command=List'">
</form>
