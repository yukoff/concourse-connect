...
{"ResultSet":
  {"totalResultsAvailable":<%= hits.size() %>,
   "totalResultsReturned":<%= hits.size() %>,
...
%>
    {"Title":"<%= StringUtils.jsStringEscape(project.getTitle()) %>"}<ccp:evaluate if="<%= i < hits.size() - 1 %>">,</ccp:evaluate>
<%
  }
%>
  ]}
