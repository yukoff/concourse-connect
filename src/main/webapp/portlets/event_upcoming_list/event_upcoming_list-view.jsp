  ...
  <c:when test="${!empty eventList}">
    <ol>
      <c:forEach var="meeting" items="${eventList}">
        ...
        <li class="eventsPortletOrderedListItem">
          <div class="leftColumn">
            <h4 class="eventsPortletHeader"><a href="${ctx}/show/${project.uniqueId}" title="<c:out value="${meeting.title}"/> link"><c:out value="${meeting.title}"/></a></h4>
            <ul class="eventsPortletUnorderedListItem">
              ...
              <c:choose>
                ...
                <c:otherwise>
                  <li class="edate">Starts ${startDate} at <ccp:tz timestamp="${meeting.startDate}" timeOnly="true"/></li>
                  <li class="edate">Ends ${endDate} at <ccp:tz timestamp="${meeting.endDate}" timeOnly="true"/></li>
                </c:otherwise>
              </c:choose>
              <c:if test="${!empty meeting.location}">
                <li class="address">
                  <address>
                    <c:out value="${meeting.location}"/>
                  </address>
                </li>
              </c:if>
            </ul>
          ...
          <div class="rightColumn">
            ...
            <ccp:rating id='${project.id}'
             ...
             --%>
              <%-- <div class="signUp">
                <c:set var="meetingId" scope="request">
                    ${meeting.id}
                </c:set>
                <c:set var="attendeeId" scope="request">
                    ${attendee.id}
                </c:set>
                <jsp:include page="event_upcoming_attend_include.jsp" />
              </div>
              --%>
          </div>
        </li>
