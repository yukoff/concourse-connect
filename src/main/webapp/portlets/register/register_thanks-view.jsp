...
<div class="registerThanks">
  <form name="register" method="post" action="<%= pageContext.getAttribute("submitContentUrl") %>">
    <input type="hidden" name="currentPage" value="thanks" />
    <div class="formContainer">
      <fieldset id="registration complete">
        <legend>Thank you for registering!</legend>
        <p>Your login information has been emailed to:</p>
        <p class="lightBlue"><c:out value="${registerBean.email}" /></p>
        <p>After you receive this email follow the enclosed instructions to login.</p>
      </fieldset>
      <c:choose>
        ...
        <c:otherwise>
          <a href="${ctx}/" class="submit">Continue</a>
        </c:otherwise>
      </c:choose>
    </div>
  </form>
