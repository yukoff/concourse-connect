...
<%
    ...
    while (movingCal.getTimeInMillis() < allocationList.getEndDate().getTime()) {
      boolean today = formatter.format(movingCal.getTime()).equals(formatter.format(todayDate));
      // Rollup the hours
      Double projectHoursValue = (Double) projectMap.getEstimatedDailyHours().get(formatter.format(movingCal.getTime()));
      String projectHours = null;
      if (projectHoursValue != null) {
        double hoursDouble = projectHoursValue.doubleValue();
        if (hoursDouble == 0) {
          projectHours = "";
        } else {
          hoursDouble = (double) Math.round(hoursDouble);
          projectHours = String.valueOf(hoursDouble);
          if (projectHours.endsWith(".0")) {
            projectHours = projectHours.substring(0, projectHours.length() - 2);
          }
        }
      }
%>
...
<%
    ...
    while (users.hasNext()) {
      ...
      AssignmentUserAllocation userAllocation = (AssignmentUserAllocation) projectMap.get(userId);
%>
...
<%
      ...
      while (movingCal.getTimeInMillis() < allocationList.getEndDate().getTime()) {
        ...
        Double hoursValue = (Double) userAllocation.getEstimatedDailyHours().get(formatter.format(movingCal.getTime()));
        String hours = null;
        if (hoursValue != null) {
        ...
        if (userAllocation.getTimesheet().getTimesheet(movingCal).getVacation()) {
