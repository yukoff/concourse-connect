...
import com.concursive.connect.web.modules.promotions.contribution.ContributionCalculationForAdsPlaced;
 ...
 */
public class ContributionCalculationForAdsPlacedTest extends AbstractConnectionPoolTest {

  protected static final int USER_ID = 1;
  //Required to insert a project
  protected static final int GROUP_ID = 1;
  
  public void testContributionCalculationForAdsPlaced() throws SQLException {

    LookupContribution lookupContribution = new LookupContribution();
    lookupContribution.setConstant("200901101230");
    lookupContribution.setDescription("Test Ads Placed Contribution");
    ...
    //Inserting Ad
    Ad newAd = new Ad();
    newAd.setProjectId(project.getId());
    newAd.setHeading("Ad Heading");
    newAd.setContent("Ad Content");
    newAd.setPublishDate(new Timestamp(System.currentTimeMillis() - 1000L * 60 * 60 * 10));
    newAd.setExpirationDate(new Timestamp(System.currentTimeMillis() - 1000L * 60 * 60 * 1));
    newAd.setEnteredBy(USER_ID);
    newAd.setModifiedBy(USER_ID);
    boolean adInserted = newAd.insert(db);
    assertTrue("Ad was not inserted", adInserted);
    assertTrue("Ad did not have an id", newAd.getId() > -1);
    ...
    ContributionCalculationForAdsPlaced contributionCalculationForAdsPlaced = new ContributionCalculationForAdsPlaced();
    contributionCalculationForAdsPlaced.process(db, lookupContribution);
