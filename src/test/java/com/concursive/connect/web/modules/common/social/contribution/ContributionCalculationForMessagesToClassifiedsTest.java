...
import com.concursive.connect.web.modules.classifieds.contribution.ContributionCalculationForMessagesToClassifieds;
 ...
 */
public class ContributionCalculationForMessagesToClassifiedsTest extends AbstractConnectionPoolTest {

  protected static final int USER_ID = 1;
  //Required to insert a project
  protected static final int GROUP_ID = 1;
  
  public void testContributionCalculationForMessagesToPromotions() throws SQLException {

    LookupContribution lookupContribution = new LookupContribution();
    lookupContribution.setConstant("200901101230");
    lookupContribution.setDescription("Test messages to promotions Contribution");
    ...
    project.setShortDescription("Project SQL Test testContributionCalculationForMessagesToPromotions");
    ...
    //Inserting a Classified
    Classified classified = new Classified();
    classified.setTitle("title");
    classified.setDescription("description1");
    ...
    classified.setExpirationDate(new Timestamp(System.currentTimeMillis() + 1000L * 60 * 60 * 24));
    boolean classifiedInserted = classified.insert(db);
    assertTrue("Classified was not inserted", classifiedInserted);
    assertTrue("Classified  did not have an id", classified.getId() > -1);

    //Inserting a Private Message
    PrivateMessage newPrivateMessage = new PrivateMessage();
    newPrivateMessage.setProjectId(project.getId());
    newPrivateMessage.setBody("Message Body");
    newPrivateMessage.setProjectId(project.getId());
    newPrivateMessage.setLinkProjectId(project.getId());
    newPrivateMessage.setLinkModuleId(Constants.PROJECT_CLASSIFIEDS_FILES);
    newPrivateMessage.setLinkItemId(classified.getId());
    newPrivateMessage.setEnteredBy(USER_ID);
    boolean messageInserted = newPrivateMessage.insert(db);
    assertTrue("message was not inserted", messageInserted);
    assertTrue("message  did not have an id", newPrivateMessage.getId() > -1);
    ...
    ContributionCalculationForMessagesToClassifieds contributionCalculationForMessagesToClassifieds = new ContributionCalculationForMessagesToClassifieds();
    contributionCalculationForMessagesToClassifieds.process(db, lookupContribution);
    ...
    //Delete test project and review
