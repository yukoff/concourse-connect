...
import com.concursive.connect.web.modules.wiki.contribution.ContributionCalculationForWikiPagesModified;
 ...
 * @created February 06, 2009
 */
public class ContributionCalculationForWikiPagesModifiedTest extends AbstractConnectionPoolTest {

  protected static final int USER_ID = 1;
  //Required to insert a project
  protected static final int GROUP_ID = 1;
  
  public void testContributionCalculationForWikiPagesModified() throws SQLException {

    LookupContribution lookupContribution = new LookupContribution();
    lookupContribution.setConstant("200901101230");
    lookupContribution.setDescription("Test Wiki Pages Modified Contribution");
    ...
    project.setShortDescription("Project SQL Test testContributionCalculationForWikiPagesModified");
    ...
    //Inserting wiki (which inserts a version too)
    Wiki newWiki = new Wiki();
    newWiki.setProjectId(project.getId());
    newWiki.setSubject("Wiki Subject");
    newWiki.setContent("Wiki Content");
    newWiki.setEnteredBy(USER_ID);
    newWiki.setModifiedBy(USER_ID);
    boolean wikiInserted = newWiki.insert(db);
    assertTrue("Wiki was not inserted", wikiInserted);
    assertTrue("Inserted wiki did not have an id", newWiki.getId() > -1);
    ...
    ContributionCalculationForWikiPagesModified contributionCalculationForWikiPagesModified = new ContributionCalculationForWikiPagesModified();
    contributionCalculationForWikiPagesModified.process(db, lookupContribution);
    ...
    //Delete test project, wiki and wiki version
