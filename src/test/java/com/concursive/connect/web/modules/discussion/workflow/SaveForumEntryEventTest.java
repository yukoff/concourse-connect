...
/**
 * Test to see if a project forum add event is recorded...
 *
 * @author Ananth
 * @created Feb 22, 2009
 */
public class SaveForumEntryEventTest extends AbstractWorkflowManagerTest {
  protected static final int GROUP_ID = 1;
  protected static final String PROCESS_NAME = "teamelements.application.project.create-forum";
  protected final static String HISTORY_TEXT = "history.text";

  public void testSaveForumEntryEvent() throws Exception {
    // Insert project
    ...
    context.setAttribute("forum", WikiLink.generateLink(forum));
    ...
    historyList.setLinkObject(ProjectHistoryList.DISCUSSION_OBJECT);
    historyList.setEventType(ProjectHistoryList.CREATE_FORUM_EVENT);
    historyList.setLinkItemId(forum.getId());
    ...
    assertEquals("Recorded event mismatch",
        "[[|" + userProfile.getId() + ":profile||" + userProfile.getTitle() + "]] created a forum [[|" + project.getId() + ":forum|" + forum.getId() + "|Dummy Forum Subject]] in [[|" + project.getId() + ":profile||Project SQL Test]]",
        history.getDescription());

    //Delete the history item because the test is done (and any other needed objects)
    history.delete(db);

    //Delete the project forum
