...
/**
 * Test to see if project rating events are being recorded..
 *
 * @author Ananth
 * @created Feb 22, 2009
 */
public class SaveProjectRatingEventTest extends AbstractWorkflowManagerTest {
  protected static final int GROUP_ID = 1;

  protected final static String HISTORY_ADD_REVIEW_TEXT = "history.addreview.text";
  protected final static String HISTORY_UPDATE_REVIEW_TEXT = "history.updatereview.text";
  ...
  public void testProjectReviewUpdate() throws Exception {
    String PROCESS_NAME = "teamelements.application.project.update-review";
    ...
    //Update the rating
    ProjectRating newRating = new ProjectRating(db, rating.getId());
    newRating.setRating(3);
    ProjectRating.save(db, newRating);

    Project userProfile = ProjectUtils.loadProject(workflowUser.getProfileProjectId());

    BusinessProcess thisProcess = (BusinessProcess) processList.get(PROCESS_NAME);
    assertTrue("Process not found...", thisProcess != null);
    context.setPreviousObject(rating);
    context.setThisObject(newRating);
    context.setProcessName(PROCESS_NAME);
    context.setProcess(thisProcess);
    context.setAttribute("userId", new Integer(USER_ID));
    context.setAttribute("user", WikiLink.generateLink(userProfile));
    ...
    historyList.setEnteredBy(USER_ID);
    historyList.setLinkObject(ProjectHistoryList.RATING_OBJECT);
    historyList.setEventType(ProjectHistoryList.UPDATE_PROFILE_REVIEW_EVENT);
    historyList.setLinkItemId(rating.getId());
    historyList.buildList(db);
    assertTrue("History event was not recorded!", historyList.size() == 1);
